...
__print(char *s)
{
	//fputs(s, stdout); // <-- 이걸로 하면, 버퍼가 달라서, 순서가 다르게 나온다. -_-;;
	// 다른 모든 printf 를 전부 fputs 로 바꾸거나.. -_-;; fprintf 로 바꿔도 될 것 같긴 한데..
	
	printf("%s", s);
}

static pn_object *
PnStdio_Print(pn_world *world, pn_object *object, pn_object *params[], int length)
{
...
static pn_object *
PnStdio_PrintLine(pn_world *world, pn_object *object, pn_object *params[], int length)
{
	PN_ASSERT(world != NULL);
	PN_ASSERT(object != NULL);
	PN_ASSERT(length == 1);
...
static pn_object *
PnStdio_ReadLine(pn_world *world, pn_object *object, pn_object *params[], int length)
{
...
pn_object *
PnStdio_CreatePrototype(pn_world *world)
{

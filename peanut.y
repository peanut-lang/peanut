...
expression          : complex_var ASSIGN simple_expr
                        { $$ = new_expression($1, $2, $3); }
                    | simple_expr
                        { $$ = $1; }
                    ;
...
funcall             : func_id '(' parameter_list ')'
                        { $$ = new_expression(NULL, $1, $3); }
                    ;
...
/**
 * insert `object` to front of chaining_list
 */
static pn_node *set_object_of_complex_var(pn_node *object, pn_node *chaining_list)
{
    PN_ASSERT(object != NULL);
